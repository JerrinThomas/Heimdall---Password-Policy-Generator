...
function getCat(x) {
  return new Promise( function(resolve, reject) {
         db.con.query(`SELECT category FROM seed_map WHERE row_id='${x+1}'`, function (error, cat, fields) {
            if (error) throw error;
            db.con.query(`SELECT count(${cat[0].category}) cnt FROM seeds_warehouse`, function (error, results, fields) {
               if (error) throw error;
...
function getCat2(x,y) {
  return new Promise( function(resolve, reject) {
         db.con.query(`SELECT category FROM seed_map WHERE row_id='${x+1}'`, function (error, cat, fields) {
            if (error) throw error;
            db.con.query(`SELECT ${cat[0].category} as s FROM seeds_warehouse WHERE seq_no = '${y}'`, function (error, seed, fields) {
               if (error) throw error;
...
function updatedb(name, email, login_time, hash, mob,pid){
  db.con.query(`INSERT INTO name_time_warehouse(uname,email,logintime,mobile,pid) VALUES('${name}','${email}','${login_time}','${mob}','${pid}')`, function (error, res1, fields) {
     if (error)
        console.log(`INSERT INTO name_time_warehouse(uname,email,logintime) VALUES('${name}','${email}','${login_time}')`);;
      db.con.query(`SELECT uid FROM name_time_warehouse WHERE uname='${name}' AND email='${email}'`, function (error, res2, fields) {
      if (error)
           console.log(`SELECT uid FROM name_time_warehouse WHERE uname='${name}' AND email='${email}'`);;
        db.con.query(`INSERT INTO hash_warehouse(uid,hashval) VALUES('${res2[0].uid}','${hash}')`, function (error, res3, fields) {
           if (error)
...
function check_hash(uname, password, color1, color2){
  return new Promise( function(resolve, reject){
    db.con.query(`SELECT uid,logintime FROM name_time_warehouse WHERE uname='${uname}'`, function (error, row, fields) {
       if (error) throw error;
       salt_hash_gen(password,row[0].logintime,color1,color2).
       then((key)=>{
            db.con.query(`SELECT hashval FROM hash_warehouse WHERE uid='${row[0].uid}'`, function (error, hash, fields) {
             if (error) throw error;
